...
def linear_interpolation(model, end_p, evaluation_f, steps=100):
    """
    Returns an approximation of the loss of the model along a linear subspace of the
    ...
    task of interest, returning the resulting loss.
    """
    library = determine_library(model, end_p)

    # avoid aliasing issues by working on fresh copies
    model_copy = copy.deepcopy(model)
    end_p_copy = copy.deepcopy(end_p)
    model_parameters = _get_parameters(model, library)

    # get distance and direction from start point to end point
    distance = _get_l2_norm(_get_displacement(model_parameters, end_p_copy, library), library)
    direction = _get_unit_vector(_get_displacement(model_parameters, end_p_copy, library), library)
...
def planar_interpolation(model, end_p_one, end_p_two, evaluation_f, steps=100):
    """
    Returns an approximation of the loss of the model along a planar subspace of the
    ...
    task of interest, returning the resulting loss.
    """
    library = determine_library(model, end_p_one, end_p_two)
    ...
    direction_one = _get_displacement(model_parameters, end_p_one_copy, library)
    direction_two = _get_displacement(model_parameters, end_p_two_copy, library)
    distance_one = _get_l2_norm(direction_one, library)
    distance_two = _get_l2_norm(direction_two, library)

    # compute and return losses
    return _compute_plane(model_copy, direction_one, direction_two, distance_one, distance_two, steps, evaluation_f,
                          library, False)


def _compute_line(model, direction, distance, steps, evaluation_f, library):
    if library == 'torch':
        return backends.torch.compute.line(model, direction, distance, steps, evaluation_f)
    else:
...
def _sample_uniform_like(vector, library, unit_vector=True):
    if library == 'torch':
        return backends.torch.compute.sample_uniform_like(vector, unit_vector)
    else:
...
def _get_advanced_normalized_vector(direction, model_parameters, norm_type, library):
    if library == 'torch':
        return backends.torch.compute.get_normalized_vector(direction, model_parameters, norm_type)
    else:
...
def _get_unit_vector(vector, library):
    if library == 'torch':
        return backends.torch.ops.unit_vector(vector)
    else:
...
def _get_displacement(point_a, point_b, library):
    if library == 'torch':
        return backends.torch.ops.vector_subtraction(point_b, point_a)
    else:
...
def _get_l2_norm(vector, library):
    if library == 'torch':
        return backends.torch.ops.l2_norm(vector)
    else:
