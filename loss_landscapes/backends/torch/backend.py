...
import loss_landscapes.backends.backend


class TorchBackend(loss_landscapes.backends.backend.Backend):
    def __init__(self):
        pass

    def line(self, model, direction, distance, steps, evaluation, agent=None, model_setter=None) -> np.ndarray:
        """
        Computes loss for discretized intervals on an one-dimensional parameter line.

        :param model: converter that wraps the model and updates its parameters
        :param direction: direction vector in parameter space
        :param distance: distance along direction to reach
        :param steps: how many steps along direction to reach distance
        :param evaluation: model evaluation function
        :param agent: a general parametric agent that encapsulates the model
        :param model_setter: a function that updates the agent to include the updated model
        """
        ...
        for _ in range(steps):
            ...
            # general parametric agent
            if model_setter is not None:
                model_setter(agent, model)
                losses.append(evaluation(agent))
            # parametric model only
            else:
                losses.append(evaluation(model))

        return np.array(losses)

    def plane(self, model, direction_one, direction_two, distance_one, distance_two, steps, evaluation, center=False,
              agent=None, model_setter=None) -> np.ndarray:
        """
        Computes loss for discretized intervals on a two-dimensional parameter plane.

        :param model: model for which parameters are to be updated
        :param direction_one: first direction vector in parameter space
        :param direction_two: second direction vector in parameter space
        :param distance_one: distance along direction to reach
        :param distance_two: distance along direction to reach
        :param steps: how many steps along direction to reach distance
        :param evaluation: model (or agent) evaluation function
        :param center: whether the start point should be the top-left or central point of the plane
        :param agent: a general parametric agent that encapsulates the model
        :param model_setter: a function that updates the agent to include the updated model
        :return: loss values along plane
        """
        # set all parameters in the model to not require gradients, which allows
        # using torch operations on the parameters
        ops.unrequire_grad__(model)
    ...
    def sample_uniform_like(self, source_parameters, unit_vector=True) -> list:
        """
    ...
    def get_normalized_vector(self, direction, model_parameters, norm_type) -> list:
        """
